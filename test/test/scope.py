...
source_path = "C:/Users/dvivi/proty-collection/proty02/assets/csv/sample2/pfugsumber.csv"
sensor_path = "C:/Users/dvivi/proty-collection/proty02/assets/csv/sample2/pfugrc.csv"
...
async def update_streaming_chart(chart):
    ...
    data_array = np.ndarray((500000,), dtype=np.float64, buffer=data_shm.buf)

    try:
        while True:
            if flag_array[1] == 1:
                data_sensor = data_array.copy()
                data_time = np.arange(0, len(data_sensor))
                chart.options['series'][0]['data'] = np.column_stack((data_time, data_sensor))
                chart.update()
                print("stream chart updated")
                flag_array[1] = 0
    ...
    with theme.frame("Oscilloscope Panel", "OK!"):
        with ui.element('div').classes('grid grid-cols-12 w-full gap-5 mt-16 mb-16'):
            with ui.element('div').classes('col-start-1 col-span-8 size-full'):
                ...
                with ui.tab_panels(tabs, value=tab_streaming).classes('w-full'):
